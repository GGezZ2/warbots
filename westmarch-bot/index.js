...
client.on("interactionCreate", async interaction => {
  ...
  if (command === "crea_pg") {
    ...
    if (existing.length >= 3) {
      return interaction.reply({ content: "Questo giocatore ha già 3 PG attivi!", ephemeral: true });
    }
  ...
  if (command === "ricompensa") {
    ...
    if (levelMsg && !process.env.LEVEL_UP_CHANNEL) {
      response += `\n\n${levelMsg}`;
    }
  ...
  if (command === "versa") {
    ...
    return interaction.reply(
      `${user.username} - PG **${pg.name}**: deposito bancario ${before} → ${after}. ` +
      `Versati **${amount}** oro. Nota: ${note}`
    );
  }
